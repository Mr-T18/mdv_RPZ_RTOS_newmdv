...
void set_CANID() {
  ...
  while((cnt_d>0) && flg==0) {
    ...
    if((digitalRead(SW_L)==HIGH) || (digitalRead(SW_R)==HIGH)) cnt_d--;
    else {
      flg=1;
      cnt_d=0; 
    }
  ...
  while(flg!=99){
    ...
    if(flg==1) {
      ...
      u8x8.draw2x2String(0, 0, "ID:");
      if (cur == 0) u8x8.setInverseFont(1); else u8x8.setInverseFont(0);
      sprintf(temp, "%d", data[0]);
      u8x8.draw2x2String(6, 0, temp);
      if (cur == 1) u8x8.setInverseFont(1); else u8x8.setInverseFont(0);
      sprintf(temp, "%d", data[1]);
      u8x8.draw2x2String(8, 0, temp);
      if (cur == 2) u8x8.setInverseFont(1); else u8x8.setInverseFont(0);
      sprintf(temp, "%d", data[2]);
      u8x8.draw2x2String(10, 0, temp);
      if (cur == 3) u8x8.setInverseFont(1); else u8x8.setInverseFont(0);

      can_id = ((data[0]*100)+(data[1]*10)+data[2]);
      if( (can_id > 254) || (can_id < 1) )  u8x8.draw2x2String(12, 0, "_");   
      else  u8x8.draw2x2String(12, 0, "*");
      
      u8x8.setInverseFont(0);

    }

    if(digitalRead(SW_L)==LOW) flg=2;
    ...
    if(digitalRead(SW_R)==LOW) flg=3;
    if((digitalRead(SW_R)==HIGH) && flg==3) {
      if(cur!=3) {   
        data[cur]++;
        if(data[cur]>9) data[cur]=0;
        if((cur==0) && (data[cur]>2)) data[cur]=0;
        delay(50);
        flg=1;
      }
      else {
        can_id=(data[0]*100)+(data[1]*10)+data[2];
        if( (can_id <= 254) && (can_id >= 1) ) flg=99;
        else flg=1; 
      }
      delay(50);
    }
...
void Control()
{
  ...
  if(ctrl_mode != 9) {
    ...
    if(RFlag != 0) { // RFlagはloop()によって立てられる
      ...
      if((localBuf[0]&0x01)!=0) dir=0;   //命令1バイト目が xxxx xx01 ならCW
      if((localBuf[0]&0x02)!=0) dir=1;   //命令1バイト目が xxxx xx10 ならCCW
...
void Motor()
{
  ...
  if (local_m_dir != pre_m_dir)
  {
    ...
    vTaskDelay(pdMS_TO_TICKS(10)); // dly10() を置き換え
    if (local_m_dir == 0) digitalWrite(MOT_DIR,LOW);
    else digitalWrite(MOT_DIR,HIGH);
  }
...
{ 
  ...
  u8x8.setCursor(4, 1);
  switch (local_ctrl_mode){ // ローカル変数を使用
    case 0:
      u8x8.print("DTY");
      break;
    case 9:
      u8x8.print("Btn");
      if(flg==1)  { // 'flg' はCore 0のControlタスク専用のためMutex不要
        u8x8.setCursor(9, 3);
        u8x8.setFont(font_c);   
        u8x8.print(" \x40 ");
        u8x8.setFont(font_n);
  ...
  u8x8.setCursor(13, 1);
  u8x8.setFont(font_c); 
  if (local_dsp_duty == 0) u8x8.print("   "); // ローカル変数
  if ((local_dsp_duty >  0 )&&(local_dsp_duty < 20)) u8x8.print("\x46  ");
  if ((local_dsp_duty >= 20)&&(local_dsp_duty < 40)) u8x8.print("\x47  ");
  if ((local_dsp_duty >= 40)&&(local_dsp_duty < 60)) u8x8.print("\x47\x46 ");
  if ((local_dsp_duty >= 60)&&(local_dsp_duty < 80)) u8x8.print("\x47\x47 ");
  if ((local_dsp_duty >= 80)&&(local_dsp_duty < 100)) u8x8.print("\x47\x47\x46");
  if (local_dsp_duty == 100) u8x8.print("\x47\x47\x47");
  u8x8.setFont(font_n);
  
  u8x8.setCursor(15, 0); 
  if(local_app!=1.0)  {u8x8.print("A");} // ローカル変数
  else                {u8x8.print(" ");}
  
  u8x8.setFont(font_c);
  
  u8x8.setCursor(4, 2);
  if(local_m_dir==0)  {if(local_m_rev==0){u8x8.print(" \x41 ");}else{u8x8.print(" \x40 ");}} // ローカル変数
  else                {if(local_m_rev==0){u8x8.print(" \x43 ");}else{u8x8.print(" \x42 ");}}
    
  u8x8.setCursor(13, 2); 
  if(local_m_rev==0)  {u8x8.print(" \x45 ");} // ローカル変数
  else                {u8x8.print(" \x44 ");}
  ...
  if(digitalRead(LMT_L)==LOW){
    ...
    u8x8.setFont(font_n);
    dsp_flg4 = 1;
    if(lmt_flg_A==0) send_can(0b00000010); // send_can()はCore 1から呼んでOK
  }else{
    lmt_flg_A=0;
    if (dsp_flg4 ==1)
    {
  ...
  if(digitalRead(LMT_H)==LOW){
    ...
    u8x8.print("X");
    dsp_flg5 = 1;   
    if(lmt_flg_B==0) send_can(0b00000001); // send_can()はCore 1から呼んでOK
  }else{
    lmt_flg_B=0;
    if (dsp_flg5 ==1)
    {
...
void showStatus(){
  ...
  if (xSemaphoreTake(xMotorDataMutex, pdMS_TO_TICKS(100)) == pdTRUE) {
    ...
    // ローカル変数を使ってシリアル出力
    Serial.print("ID_RCV: "); Serial.print(local_ID_RCV); Serial.print("   ");
    Serial.print("  CAN_ID: ");Serial.print(can_id);
    Serial.print("m_dir: ");Serial.print(local_m_dir);
    Serial.print(",  m_rev: ");Serial.print(local_m_rev);Serial.print("   ");
    Serial.print(", ctrl_mode: ");Serial.print(local_ctrl_mode);Serial.print("   ");
    Serial.println("");
    Serial.println("");
  }
  #endif
